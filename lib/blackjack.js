  ...
  constructor(deckCount){
    ...
    for (var i = 0; i < deckCount; i++) {
      for (let suit in suits) {
        for (let val in deck.valueStrings) {
          this.cards.push(new Card(suits[suit], deck.valueStrings[val], deck.blackjackValues[val]));
        }
      }
    }
...
class Dealer {
  ...
  payBets(currentPlayer){
    if (currentPlayer.hand.beats(this.hand)) {
      currentPlayer.payWinnings(JSON.parse(this.bet) * 2);
    } else if (currentPlayer.hand.points() === this.hand.points()) {
      currentPlayer.payWinnings(JSON.parse(this.bet));
    }
  }
}
...
function getBetCallback(e) {
  ...
  score.innerHTML = "Bankroll: $" + this.player1.bankroll;
...
function renderHit(game) {
  ...
  popup.innerHTML = "Expected Card Value: " + expectedValue.toFixed(2);
}
...
function dealerCallback(e) {
  ...
  this.game.dealer.hand.cards.forEach((card, idx) => {
    ...
    const maxBet = document.getElementById("quantity");
    maxBet.max = this.player1.bankroll;
...
function restartCallback(e) {
  ...
  if (this.game.deck.cards.length < 15 || this.game.player1.bankroll === 0) {
    const ending = document.getElementById('endModal');
    ending.style.display = "block";

    if (this.game.player1.bankroll === 0) {
      const broke = document.getElementById('ending-message');
      broke.textContent = "You have ran out of money!";
    } else {
      const noCards = document.getElementById('ending-message');
      noCards.textContent = "You have reached the end of the deck!";
    }

    const footer = document.getElementsByClassName("end-modal-footer")[0];
    footer.innerHTML = "Final score is $" + this.game.player1.bankroll;

    const span = document.getElementsByClassName("close-end")[0];
