...
class PlayerAI {
  constructor(dealerTop, playerHand, points, expectedValue) {
    ...
    this.dealerProbability = DEALERTABLE[valueStrings.indexOf(this.dealerTop.valueString)];
  ...
  hitdecision() {
    ...
    if (this.points < dealerExpectedValue) {
      if ((this.points + this.expectedValue) <= 21) {
        ...
        return false;
        // being conservative and not hitting when there is a grater than 50% chance of busting is better!
        // return ((this.expectedValue + this.points - 21) < (dealerExpectedValue - this.points));
      }
    } else {
      return false;
    }
  ...
  // if not add expectedValue and compare outcomes between hit and stay
